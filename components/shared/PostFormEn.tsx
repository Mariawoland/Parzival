...
import { createPost } from "@/lib/actions/postEn.actions"
...
const PostFormEn = ({ userId, post, postId }: PostFormProps) => {
    ...
    async function onSubmit(values: z.infer<typeof PostFormSchema>) {
        ...
        try {
            ...
            if (newPost) {
                form.reset();
                router.push(`/postsEn/${newPost._id}`)
            }
        } catch (error) {
            console.log(error);
        }
    ...
    return (
        <>
            <div className="lg:w-1/2 w-full px-5 m-auto mt-10">
                <h1 className="text-lg font-black text-two-1 pb-5 ps-1">პოსტის დამატება inglisurad</h1>
                <Form {...form}>
